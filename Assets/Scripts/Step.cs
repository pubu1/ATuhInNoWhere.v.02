...
public class Step : MonoBehaviour
{
    ...
    private bool CanStepToPosition(Vector2 currentPosition, Vector2 targetPosition, string tempNextKey)
    {
        ...
        {
            int tempTargetMap = (int)targetPosition.x / 100;
            ...
            button.CheckCurrentStep(playerScript, gameManager.PlayGridList[tempTargetMap][tempXTarget, tempYTarget], gameManager.WireMap);
        }
        else if (gameManager.PlayGridList[currentMap][xCurrent, yCurrent].tag == "Teleporter")
        {
            ...
            totalCheck = teleporter.CheckNextStepInside(playerScript, gameManager.PlayGridList[tempTargetMap][tempXTarget, tempYTarget], gameManager.WireMap);
            ...
            if (totalCheck)
            {
                if (teleporter.IsTeleport(tempTargetPosition))
                {
                    ...
                    if (gameManager.PlayGridList[tempTargetMap][xTarget, yTarget].tag == "Socket")
                    {
                        Socket socket = gameManager.PlayGridList[tempTargetMap][xTarget, yTarget].GetComponent<Socket>();
                        if (socket.CheckSocketEndPoint(playerScript))
                        {
                            socket.ChangePlayerAttrEndPoint(playerScript);
                            GenerateWire(tempTargetMap, xCurrent, yCurrent, "Wire", null);
                            gameManager.Score++;
                        }
        ...
        else if (gameManager.PlayGridList[currentMap][xTarget, yTarget].tag == "EscButton")
        {
            EscButton button = gameManager.MapGridList[currentMap][xTarget, yTarget].GetComponent<EscButton>();
            button.IsActive = true;
            ...
            {          
                UpdateLocation();
                GenerateWire(currentMap, xCurrent, yCurrent, "Wire", null);     
                if(!playerScript.IsNotPickWire) button.HasPipeOn = true;          
            }
        }
